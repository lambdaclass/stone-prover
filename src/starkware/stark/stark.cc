...
/*
  Translates indices from fri_params->fft_bases to evaluation_domain. Returns a
  vector of pairs (coset_index, offset) s.t. fri_domain[fri_query] =
  evaluation_domain.GetFieldElementAt(coset_index, offset). Currently, FRI
  ignores the offset, so this equation is true only up to the offset correction
  between evaluation domain and fri_params->fft_bases.
*/
std::vector<std::pair<uint64_t, uint64_t>>
FriQueriesToEvaluationDomainQueries(const std::vector<uint64_t> &fri_queries,
...
*/
FieldElementVector GetInteractionElements(const size_t n_interaction_elements,
                                          const Field &field,
                                          Channel *channel) {
...
static std::unique_ptr<FftBases> GenerateCompositionBases(const Field &field,
                                                          const Air &air) {
  ...
  auto bases = InvokeFieldTemplateVersion(
      [&](auto field_tag) -> std::unique_ptr<FftBases> {
        ...
        return std::make_unique<MultiplicativeFftBases<
            FieldElementT, MultiplicativeGroupOrdering::kBitReversedOrder>>(
            log_size, offset);
      },
      field);
...
void StarkParameters::VerifyCompatibleDomains() {
  ...
  ASSERT_RELEASE(all_offsets.size() >= n_relevant_cosets,
                 "Not enough offsets in evaluation domain.");
  ASSERT_RELEASE(cosets.size() == n_relevant_cosets,
                 "Number of cosets in composition_eval_bases is wrong.");
  ...
  ASSERT_RELEASE(eval_domain_group.Size() == fft_elements_group.Size(),
                 "Groups have difference sizes.");
  ASSERT_RELEASE(eval_domain_group.GetFieldElementAt(0) ==
                     fft_elements_group.GetFieldElementAt(0),
                 "The first elements in the groups are not the same.");
  ASSERT_RELEASE(eval_domain_group.GetFieldElementAt(1) ==
                     fft_elements_group.GetFieldElementAt(1),
                 "The second elements in the groups are not the same.");
}

StarkParameters::StarkParameters(const Field &field,
                                 const bool use_extension_field,
                                 size_t n_evaluation_domain_cosets,
                                 size_t trace_length,
                                 MaybeOwnedPtr<const Air> air,
                                 MaybeOwnedPtr<FriParameters> fri_params)
    : field(field), use_extension_field(use_extension_field),
      ...
      air(std::move(air)), composition_eval_bases(TakeOwnershipFrom(
                               GenerateCompositionBases(field, *this->air))),
      fri_params(std::move(fri_params)) {
  ASSERT_RELEASE(IsPowerOfTwo(n_evaluation_domain_cosets),
                 "The number of cosets must be a power of 2.");
  if (use_extension_field) {
    ASSERT_RELEASE(IsExtensionField(field),
                   "Use extension field is true but the field is not of type "
                   "extension field.");
  }

  // Check that the fri_step_list and last_layer_degree_bound parameters are
  // consistent with the trace length. This is the expected degree in out of
  // domain sampling.
  const uint64_t expected_fri_degree_bound =
      GetFriExpectedDegreeBound(*this->fri_params);
  const uint64_t stark_degree_bound = trace_length;
  ASSERT_RELEASE(expected_fri_degree_bound == stark_degree_bound,
                 "Fri parameters do not match stark degree bound. Expected FRI "
                 "degree from "
                 "FriParameters: " +
...
StarkParameters StarkParameters::FromJson(const JsonValue &json,
                                          const Field &field,
                                          MaybeOwnedPtr<const Air> air,
                                          const bool use_extension_field) {
  ...
  auto bases = InvokeFieldTemplateVersion(
      [&](auto field_tag) -> std::unique_ptr<FftBases> {
        ...
        return std::make_unique<FftBasesDefaultImpl<FieldElementT>>(
            MakeFftBases(log_trace_length + log_n_cosets,
                         FieldElementT::One()));
      },
      field);

  FriParameters fri_params = FriParameters::FromJson(
      json["fri"], TakeOwnershipFrom(std::move(bases)), field);

  return StarkParameters(field, use_extension_field, n_cosets, trace_length,
                         std::move(air), UseMovedValue(std::move(fri_params)));
}
...
StarkProverConfig StarkProverConfig::FromJson(const JsonValue &json) {
  const bool store_full_lde =
      json["cached_lde_config"]["store_full_lde"].AsBool();
  const bool use_fft_for_eval =
      json["cached_lde_config"]["use_fft_for_eval"].AsBool();
  ...
  const size_t n_out_of_memory_merkle_layers =
      json["n_out_of_memory_merkle_layers"].AsSizeT();

  const JsonValue fri_prover_config = json["fri_prover"];
  size_t max_non_chunked_layer_size =
  ...
  if (fri_prover_config.HasValue()) {
    const JsonValue max_layer_size =
...
void StarkProver::PerformLowDegreeTest(const CompositionOracleProver &oracle) {
  ...
  // Check that the fri_step_list and last_layer_degree_bound parameters are
  // consistent with the oracle degree bound.
  const uint64_t expected_fri_degree_bound =
      GetFriExpectedDegreeBound(*params_->fri_params);
  const uint64_t oracle_degree_bound =
      oracle.ConstraintsDegreeBound() * params_->TraceLength();
  ASSERT_RELEASE(
  ...
  FriProver::FirstLayerCallback first_layer_queries_callback =
      [this, &oracle](const std::vector<uint64_t> &fri_queries) {
        ...
        const auto queries = FriQueriesToEvaluationDomainQueries(
            fri_queries, params_->TraceLength());
  ...
  FriProver fri_prover(UseOwned(channel_), UseOwned(table_prover_factory_),
                       ...
                       std::move(composition_polynomial_evaluation),
...
StarkProver::CommitOnTrace(Trace &&trace, const FftBases &bases,
                           bool bit_reverse,
  ...
  CommittedTraceProver committed_trace(config_->cached_lde_config,
                                       UseOwned(&params_->evaluation_domain),
                                       trace.Width(), *table_prover_factory_);
  committed_trace.Commit(std::move(trace), bases, bit_reverse);
  return committed_trace;
}

CompositionOracleProver
StarkProver::OutOfDomainSamplingProve(CompositionOracleProver original_oracle) {
  ...
  // NOLINTNEXTLINE.
  auto [broken_uncommitted_trace, broken_bases] =
      oods::BreakCompositionPolynomial(composition_eval, n_breaks,
                                       *params_->composition_eval_bases);
  breaker_block.CloseBlock();

  // The resulting evaluations are on a domain which may have a different offset
  // than the trace. broken_bases represents that domain. It should have the
  // same basis, but a different offset than the trace.
  ASSERT_RELEASE(params_->evaluation_domain.Bases().At(0).BasisSize() ==
  ...
  auto boundary_air = oods::CreateBoundaryAir(
      field, params_->evaluation_domain.Group().Size(),
  ...
  auto ods_composition_polynomial = CreateCompositionPolynomial(
      channel_.get(), field, params_->evaluation_domain.TraceGenerator(),
      *boundary_air);
...
void StarkProver::ValidateFirstTraceSize(const size_t n_rows,
                                         const size_t n_columns) {
  ASSERT_RELEASE(params_->evaluation_domain.Group().Size() == n_rows,
                 "Trace length parameter " + std::to_string(n_rows) +
                     ...
                     ".");
  ASSERT_RELEASE(params_->air->GetNColumnsFirst() == n_columns,
...
void StarkProver::ProveStark(std::unique_ptr<TraceContext> trace_context) {
  ...
  // Add first committed trace.
  {
    ...
    CommittedTraceProver committed_trace(
        CommitOnTrace(std::move(trace), params_->evaluation_domain.Bases(),
                      true, "Commit on trace"));
  ...
  if (interaction_params.has_value()) {
    ASSERT_RELEASE(!params_->use_extension_field,
                   "Extension field is not implemented for interaction.");
    ...
    VLOG(0) << "Trace cells count:\nLog number of rows: " +
                   std::to_string(SafeLog2(trace_length)) +
                   "\nNumber of first trace columns: " +
                   std::to_string(first_trace_width) +
                   "\nNumber of interaction columns: " +
                   std::to_string(interaction_trace.Width()) +
                   "\nTotal trace cells: " +
                   std::to_string(trace_length * trace_width)
            << std::endl;
...
StarkVerifier::ReadTraceCommitment(const size_t n_columns,
                                   bool should_verify_base_field) {
  ASSERT_RELEASE(!should_verify_base_field || IsExtensionField(params_->field),
                 "The parameter should_verify_base_field is true but the field "
                 "is not in the form of "
                 "ExtensionFieldElement<>.");
  CommittedTraceVerifier trace_verifier(UseOwned(&params_->evaluation_domain),
                                        n_columns, *table_verifier_factory_,
...
void StarkVerifier::PerformLowDegreeTest(
    const CompositionOracleVerifier &oracle) {
  AnnotationScope scope(channel_.get(), "FRI");
  ...
  FriVerifier fri_verifier(
      UseOwned(channel_), UseOwned(table_verifier_factory_),
...
CompositionOracleVerifier StarkVerifier::OutOfDomainSamplingVerify(
  ...
  {
    trace_verifier.emplace(UseOwned(&params_->evaluation_domain),
  ...
  auto boundary_conditions = oods::VerifyOods(
      ...
      *params_->composition_eval_bases, params_->use_extension_field);

  auto boundary_air = oods::CreateBoundaryAir(
      params_->evaluation_domain.GetField(),
      params_->evaluation_domain.Group().Size(),
      ...
      std::move(boundary_conditions));
  {
    ...
    CompositionOracleVerifier composition_oracle(
        UseOwned(&params_->evaluation_domain), std::move(traces), boundary_mask,
        TakeOwnershipFrom(std::move(boundary_air)),
        TakeOwnershipFrom(std::move(ods_composition_polynomial)),
        channel_.get());
...
void StarkVerifier::VerifyStark() {
  ...
  // Create a FieldCommitmentSchemeVerifier for the decommitment.
  {
    ...
    CommittedTraceVerifier first_trace_verifier(ReadTraceCommitment(
        params_->air->GetNColumnsFirst(), skip_assert_for_extension_field_test_
                                              ? false
                                              : params_->use_extension_field));
  ...
  if (interaction_params.has_value()) {
    ...
    current_air = TakeOwnershipFrom(params_->air->WithInteractionElements(
        GetInteractionElements(interaction_params->n_interaction_elements,
                               params_->field, channel_.get())));
  ...
  CompositionOracleVerifier composition_oracle(
      UseOwned(&params_->evaluation_domain), std::move(traces),
      current_air->GetMask(), UseOwned(current_air.get()),
      TakeOwnershipFrom(std::move(composition_polynomial_)), channel_.get());
