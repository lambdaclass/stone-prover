...
std::pair<Trace, std::unique_ptr<FftBases>>
BreakCompositionPolynomial(const ConstFieldElementSpan &composition_evaluation,
                           size_t n_breaks, const FftBases &bases) {
  ...
  return {Trace::CopyFrom(output_spans),
          bases.FromLayerAsUniquePtr(log_n_breaks)};
}

std::unique_ptr<Air>
CreateBoundaryAir(const Field &field, uint64_t trace_length, size_t n_columns,
                  std::vector<std::tuple<size_t, FieldElement, FieldElement>>
...
std::vector<std::tuple<size_t, FieldElement, FieldElement>>
ProveOods(ProverChannel *channel,
          const CompositionOracleProver &original_oracle,
          const CommittedTraceProverBase &broken_trace,
  ...
  const FieldElement &trace_gen =
      original_oracle.EvaluationDomain().TraceGenerator();
  if (use_extension_field) {
    ASSERT_RELEASE(GetFrobenius(trace_gen) == trace_gen,
                   "trace_gen not in base field.");
  }
  std::vector<std::tuple<size_t, FieldElement, FieldElement>>
      boundary_constraints;
  const FieldElement point =
      channel->GetRandomFieldElementFromVerifier(field, "Evaluation point");
  const std::optional<FieldElement> conj_point =
      use_extension_field ? std::make_optional(GetFrobenius(point))
                          : std::nullopt;
  ...
  // OODS trace side.
  {
    ...
    std::printf("\n");
    std::printf("Starting mask bytes appending...\n");
    // Send values. This loop also creates the LHS of the boundary constraints
    // to be returned.
    for (size_t i = 0; i < trace_evaluation_at_mask.Size(); ++i) {
      ...
      boundary_constraints.emplace_back(column_index, point * row_element,
                                        trace_eval_at_idx);

      // In case use_extension_field is true, add a corresponding boundary
      // constarint on the conjugate element to guarantee that the trace is
      // defined over the base field. This is done only once for each column.
      if (use_extension_field && !cols_seen[column_index]) {
        cols_seen[column_index] = true;
        const auto conj_trace_eval_at_idx_value =
            GetFrobenius(trace_eval_at_idx);
        boundary_constraints.emplace_back(column_index,
                                          (*conj_point) * row_element,
                                          conj_trace_eval_at_idx_value);
      }
    }
    std::printf("End mask bytes appending\n");
  }
  ...
  // OODS broken side.
  {
    ...
    FieldElementVector broken_evaluation =
        FieldElementVector::MakeUninitialized(field, n_breaks);
    broken_trace.EvalMaskAtPoint(broken_eval_mask, point_transformed,
                                 broken_evaluation);
    std::printf("\n");
    std::printf("Start appending broken composition poly evaluations...\n");
    // Send values. This loops also creates the RHS of the boundary constraints.
    for (size_t i = 0; i < broken_evaluation.Size(); ++i) {
      channel->SendFieldElement(broken_evaluation.At(i),
                                std::to_string(trace_mask_size + i));

      // Assuming all broken_column appear right after trace columns.
      boundary_constraints.emplace_back(original_oracle.Width() + i,
                                        point_transformed,
                                        broken_evaluation.At(i));
    }
    std::printf("End appending broken composition poly evaluations\n");
  }
...
std::vector<std::tuple<size_t, FieldElement, FieldElement>>
VerifyOods(const ListOfCosets &evaluation_domain, VerifierChannel *channel,
  ...
  // OODS trace side.
  // Receive values. This loop also creates the LHS of the boundary constraints
  // to be returned.
  const auto &mask = original_oracle.GetMask();
  const size_t trace_mask_size = mask.size();
  FieldElementVector original_oracle_mask_evaluation =
      FieldElementVector::Make(field);
  original_oracle_mask_evaluation.Reserve(mask.size());
  std::vector<bool> cols_seen(original_oracle.Width(), false);

  for (size_t i = 0; i < mask.size(); ++i) {
    const FieldElement value =
        channel->ReceiveFieldElement(field, std::to_string(i));
    const auto &[row_offset, column_index] = mask[i];
    original_oracle_mask_evaluation.PushBack(value);
    boundary_constraints.emplace_back(column_index,
                                      point * trace_gen.Pow(row_offset), value);
    if (use_extension_field && !cols_seen[column_index]) {
      cols_seen[column_index] = true;
      const auto conj_trace_eval_at_idx_value = GetFrobenius(value);

      boundary_constraints.emplace_back(
          column_index, (*conj_point) * trace_gen.Pow(row_offset),
          conj_trace_eval_at_idx_value);
    }
  }

  const FieldElement trace_side_value =
  ...
  auto poly_break =
      MakePolynomialBreak(composition_eval_bases, SafeLog2(n_breaks));

  // OODS broken side.
  // Receive values. This loop also creates the RHS of the boundary constraints
  // to be returned.
  const FieldElement point_transformed = point.Pow(n_breaks);
  ...
  for (size_t i = 0; i < n_breaks; ++i) {
    const FieldElement value = channel->ReceiveFieldElement(
        field, std::to_string(trace_mask_size + i));
  ...
  const FieldElement broken_side_value =
      poly_break->EvalFromSamples(broken_evaluation, point);

  ASSERT_RELEASE(trace_side_value == broken_side_value,
