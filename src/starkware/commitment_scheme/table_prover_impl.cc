...
template <typename T>
bool VerifyAllColumnsSameLength(
    const std::vector<gsl::span<const T>> &columns) {
...
/*
  Returns serialization of field elements in table, represented by a vector of
  columns. It stores all elements from same row in a consecutive block of bytes,
  and keeps the order of rows, and the order of columns inside each row.
  Formally, if each field element takes 'b' bytes, and there are 'c' columns,
  the element from column 'x' and row 'y' is stored at 'b' bytes, starting of
  index
  '(y*c+x)*b'.
*/
...
std::vector<std::byte> SerializeFieldColumnsImpl(
  ...
  ASSERT_RELEASE(VerifyAllColumnsSameLength(columns),
...
*/
std::vector<std::byte>
SerializeFieldColumns(gsl::span<const ConstFieldElementSpan> segment) {
...
void TableProverImpl::AddSegmentForCommitment(
  ...
  commitment_scheme_->AddSegmentForCommitment(SerializeFieldColumns(segment),
                                              segment_index);
}
...
std::vector<uint64_t> TableProverImpl::StartDecommitmentPhase(
  ...
  std::vector<uint64_t> rows_to_request(all_query_rows_.begin(),
                                        all_query_rows_.end());

  // Add rows requested by the inner commitment scheme.
  std::copy(requested_elements.begin(), requested_elements.end(),
            std::back_inserter(rows_to_request));

  ASSERT_RELEASE(!HasDuplicates(gsl::make_span(requested_elements)),
                 "Found duplicate row indices in requested_elements.");

  return rows_to_request;
}

void TableProverImpl::Decommit(
    gsl::span<const ConstFieldElementSpan> elements_data) {
  // elements_data is a 2D array (indexed by column and then row). The first
  // rows refer to all_query_rows_ and the last rows refer to rows requested by
  // the inner commitment scheme.
  ...
  // which all cells are integrity queries, will not appear at all in this set.
  ...
  for (size_t i = 0; row_it != all_query_rows_.end(); i++, row_it++) {
    for (size_t col = 0; col < n_columns_; col++) {
      ...
      ASSERT_RELEASE(*to_transmit_it == query_loc,
                     "Expected to transmit " + to_transmit_it->ToString() +
                         " but found " + query_loc.ToString());
      channel_->SendFieldElement(elements_data[col][i],
                                 ElementDecommitAnnotation(query_loc));
      to_transmit_it++;
    }
  }
