...
MerkleCommitmentSchemeProver<HashT>::MerkleCommitmentSchemeProver(
    ...
    : n_elements_(n_elements), channel_(channel),
      // Initialize the tree with the number of elements, each element is the
      // hash stored in a leaf.
...
void MerkleCommitmentSchemeProver<HashT>::AddSegmentForCommitment(
  ...
  ASSERT_RELEASE(
      ...
      "Segment size is " + std::to_string(segment_data.size()) +
          " instead of the expected " +
          std::to_string(kSizeOfElement * SegmentLengthInElements()));

  tree_.AddData(BytesAsHash<HashT>(segment_data, kSizeOfElement),
                segment_index * SegmentLengthInElements());
}

template <typename HashT> void MerkleCommitmentSchemeProver<HashT>::Commit() {
  // After adding all segments, all inner tree nodes that are at least
  // (tree_height - log2(n_elements_in_segment_)) far from the root - were
  // already computed.
  size_t tree_height = SafeLog2(tree_.k_data_length);
  const HashT commitment =
      tree_.GetRoot(tree_height - SafeLog2(SegmentLengthInElements()));
  channel_->SendCommitmentHash(commitment, "Commitment");
}

template <typename HashT>
std::vector<uint64_t>
MerkleCommitmentSchemeProver<HashT>::StartDecommitmentPhase(
...
template <typename HashT>
void MerkleCommitmentSchemeProver<HashT>::Decommit(
    gsl::span<const std::byte> elements_data) {
...
bool MerkleCommitmentSchemeVerifier<HashT>::VerifyIntegrity(
  ...
  for (auto const &element : elements_to_verify) {
    ...
    ASSERT_RELEASE(element.second.size() == HashT::kDigestNumBytes,
                   "Element size mismatches.");
    hashes_to_verify[element.first] = HashT::InitDigestTo(element.second);
  }
  // Verify decommitment.
  return MerkleTree<HashT>::VerifyDecommitment(hashes_to_verify, n_elements_,
                                               *commitment_, channel_);
}
