...
void FriCommittedLayerByCallback::Decommit(
    const std::vector<uint64_t> &queries) {
  (*layer_queries_callback_)(
      SecondLayerQeuriesToFirstLayerQueries(queries, fri_step_));
}
...
FriCommittedLayerByTableProver::FriCommittedLayerByTableProver(
    ...
    size_t layer_num)
    : FriCommittedLayer(fri_step), fri_layer_(std::move(layer)),
      params_(params), layer_num_(layer_num) {
  uint64_t chunk_size = fri_layer_->ChunkSize();
  uint64_t n_chunks = SafeDiv(fri_layer_->LayerSize(), chunk_size);
  table_prover_ = table_prover_factory(n_chunks, chunk_size / Pow2(fri_step_),
                                       Pow2(fri_step_));
  ASSERT_RELEASE(table_prover_,
                 "No table prover for current layer (probably last layer)");
  Commit();
}

FriCommittedLayerByTableProver::ElementsData
FriCommittedLayerByTableProver::EvalAtPoints(
...
void FriCommittedLayerByTableProver::Commit() {
  ...
  for (uint64_t index = 0; index < n_chunks; ++index) {
    ConstFieldElementSpan chunk =
        fri_layer_->GetChunk(storage.get(), chunk_size, index);
    table_prover_->AddSegmentForCommitment({chunk}, index, Pow2(fri_step_));
  }
  table_prover_->Commit();
}

void FriCommittedLayerByTableProver::Decommit(
    const std::vector<uint64_t> &queries) {
  std::set<RowCol> layer_data_queries, layer_integrity_queries;
  NextLayerDataAndIntegrityQueries(queries, params_, layer_num_,
                                   &layer_data_queries,
                                   &layer_integrity_queries);
  std::vector<uint64_t> required_row_indices =
      table_prover_->StartDecommitmentPhase(layer_data_queries,
                                            layer_integrity_queries);
